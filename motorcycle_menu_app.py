...
# نام فایل مانیفست نسخه‌ها روی گیت‌هاب (کنار پوشه‌های Diag_Menu و Diag_Database)
EXCEL_VERSIONS_FILE = "excel_versions.json"
# کلیدهای تنظیمات مربوط به به‌روزرسانی از گیت‌هاب
CFG_GITHUB_RAW_BASE = "github_raw_base"   # مثال: https://raw.githubusercontent.com/USER/REPO/main
CFG_GITHUB_TOKEN = "github_token"         # اختیاری؛ برای ریپوی خصوصی
CFG_LOCAL_EXCEL_VERSIONS = "local_excel_versions"  # دیکشنری مسیرنسبی -> نسخه/هش

# مسیر پیش‌فرضِ پوشه‌ی اکسل‌های دیاگ (همیشه همینه، ولی چون ممکنه روی
# کامپیوترهای مختلف کمی فرق کنه، از کارت «تنظیمات» قابل تغییره).
DEFAULT_DIAG_FOLDER = r"D:\0Repository\Diag_Base\MCU\my_library\Excel Files\Diag_Menu"
DEFAULT_DATABASE_FOLDER = r"D:\0Repository\Diag_Base\MCU\my_library\Excel Files\Diag_Database"
...
# ---------------------------------------------------------------------------
# به‌روزرسانی خودکار فایل‌های اکسل از گیت‌هاب
# ---------------------------------------------------------------------------
# روی گیت‌هاب یک فایل excel_versions.json قرار می‌دهید (نمونه در README).
# برنامه هنگام باز شدن (یا با دکمه در تنظیمات) آن را می‌خواند، با نسخه‌های
# محلی مقایسه می‌کند و در صورت وجود نسخهٔ جدید، از کاربر تأیید می‌گیرد و
# فایل‌های تغییرکرده را دانلود و جایگزین می‌کند.
# ---------------------------------------------------------------------------

def _file_sha256(path, chunk_size=1024 * 1024):
    """هش SHA-256 یک فایل محلی (برای مقایسه با مانیفست گیت‌هاب)."""
    h = hashlib.sha256()
    try:
...
def _http_get_bytes(url, token=None, timeout=30):
    """دانلود بایت‌ها از یک URL (با پشتیبانی توکن اختیاری برای ریپوی خصوصی)."""
...
def _http_get_json(url, token=None, timeout=20):
    raw = _http_get_bytes(url, token=token, timeout=timeout)
    return json.loads(raw.decode("utf-8"))


def fetch_remote_excel_manifest(base_url, token=None):
    """
    مانیفست نسخه‌ها را از گیت‌هاب می‌خواند.
    base_url مثال:
      https://raw.githubusercontent.com/USERNAME/REPO/main
    انتظار می‌رود فایل excel_versions.json در همان ریشه باشد.
    """
    base = (base_url or "").rstrip("/")
    if not base:
        return None
    url = f"{base}/{EXCEL_VERSIONS_FILE}"
    return _http_get_json(url, token=token)


def find_excel_updates(manifest, local_versions, diag_folder, database_folder):
    """
    لیست فایل‌هایی که روی گیت‌هاب جدیدتر از نسخهٔ محلی‌اند را برمی‌گرداند.
    هر آیتم: {
      "rel_path": "Diag_Menu/18_MotorCycle.xlsm",
      "remote_version": "...",
      "remote_sha256": "...",
      "local_path": "D:\\...\\18_MotorCycle.xlsm",
      "folder_key": "diag" | "database"
    }
    """
    ...
    for rel_path, meta in files.items():
        ...
        remote_ver = str(meta.get("version") or meta.get("sha256") or "")
        remote_sha = (meta.get("sha256") or "").lower()
        local_meta = (local_versions or {}).get(rel_path) or {}
        local_ver = str(local_meta.get("version") or "")
        local_sha = (local_meta.get("sha256") or "").lower()

        # تعیین مسیر محلی
        lower = rel_path.lower()
        if lower.startswith("diag_menu/") or lower.startswith("diag_menu\\"):
            folder = diag_folder
            sub = rel_path.split("/", 1)[-1] if "/" in rel_path else rel_path
            folder_key = "diag"
        elif lower.startswith("diag_database/") or lower.startswith("diag_database\\"):
            folder = database_folder
            sub = rel_path.split("/", 1)[-1] if "/" in rel_path else rel_path
            folder_key = "database"
        else:
            # پیش‌فرض: داخل پوشهٔ دیاگ
            folder = diag_folder
            sub = rel_path
            folder_key = "diag"

        local_path = os.path.join(folder, sub) if folder else None
        if not local_path:
            continue

        # اگر فایل محلی وجود ندارد → نیاز به دانلود
        needs = False
        if not os.path.isfile(local_path):
            needs = True
        elif remote_sha and local_sha and remote_sha != local_sha:
            needs = True
        elif remote_sha and os.path.isfile(local_path):
            current_sha = _file_sha256(local_path)
            if current_sha and current_sha.lower() != remote_sha:
                needs = True
        elif remote_ver and local_ver and remote_ver != local_ver:
            needs = True
        ...
        if needs:
            updates.append({
                "rel_path": rel_path,
                "remote_version": remote_ver or remote_sha[:12] or "?",
...
def download_excel_file(base_url, rel_path, dest_path, token=None, timeout=120):
    ...
    """
    base = (base_url or "").rstrip("/")
    url = f"{base}/{rel_path.replace(chr(92), '/')}"
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # سازگاری با نسخه‌های قبلی برنامه: کلید قدیمی excel_path/motorcycle_excel_path نادیده گرفته می‌شه،
        # چون الان همه‌چیز از یک پوشه‌ی مشترک خونده می‌شه.
        self.diag_folder = self.config.get("diag_folder") or DEFAULT_DIAG_FOLDER
        self.database_folder = self.config.get("database_folder") or DEFAULT_DATABASE_FOLDER
        ...
        # هر بار برنامه باز می‌شه، دیتای موتورسیکلت دوباره از فایل اکسل
        # خونده می‌شه تا کاربر همیشه آخرین نسخه رو ببینه (نیازی به رفرش دستی نیست).
        # دیتای هر شرکتِ خودروساز هم اولین باری که انتخاب بشه (هر بار که
        # برنامه تازه باز شده باشه) از نو خونده می‌شه.
        self._load_motorcycle_data()
        self._screen_refresh()

        # بررسی به‌روزرسانی اکسل‌ها از گیت‌هاب (بعد از نمایش UI، در پس‌زمینه)
        self.after(1500, self._schedule_excel_update_check)

    def _schedule_excel_update_check(self):
        """اجرای بررسی آپدیت در ترد جدا تا UI قفل نشود."""
        base = (self.config.get(CFG_GITHUB_RAW_BASE) or "").strip()
        if not base:
            return
        def worker():
            try:
                self._check_excel_updates(silent_if_none=True)
            except Exception:
                pass
    ...
    def _check_excel_updates(self, silent_if_none=False, parent=None):
        """
        مانیفست گیت‌هاب را می‌خواند، فایل‌های جدید را پیدا می‌کند و در صورت
        تأیید کاربر دانلود می‌کند. از ترد پس‌زمینه صدا زده می‌شود؛ UI با
        self.after به main thread منتقل می‌شود.
        """
        parent = parent or self
        base = (self.config.get(CFG_GITHUB_RAW_BASE) or "").strip()
        token = (self.config.get(CFG_GITHUB_TOKEN) or "").strip() or None
        if not base:
            if not silent_if_none:
                self.after(0, lambda: messagebox.showinfo(
                    "به‌روزرسانی",
                    "آدرس گیت‌هاب (raw base URL) در تنظیمات مشخص نشده است.\n"
                    "مثال:\nhttps://raw.githubusercontent.com/USERNAME/REPO/main",
                    parent=parent))
            return

        try:
            manifest = fetch_remote_excel_manifest(base, token=token)
        except urllib.error.HTTPError as e:
            if not silent_if_none:
                self.after(0, lambda: messagebox.showerror(
                    "خطا",
                    f"خطا در دریافت مانیفست از گیت‌هاب (HTTP {e.code}).\n"
                    "آدرس یا توکن را بررسی کنید.",
                    parent=parent))
            return
        except Exception as e:
            if not silent_if_none:
                self.after(0, lambda: messagebox.showerror(
                    "خطا",
                    f"ارتباط با گیت‌هاب برقرار نشد:\n{e}",
                    parent=parent))
            return

        local_versions = self.config.get(CFG_LOCAL_EXCEL_VERSIONS) or {}
        updates = find_excel_updates(
            manifest, local_versions, self.diag_folder, self.database_folder)

        if not updates:
            if not silent_if_none:
                ver = (manifest or {}).get("version") or "—"
                self.after(0, lambda: messagebox.showinfo(
                    "به‌روزرسانی",
                    f"همهٔ فایل‌های اکسل به‌روز هستند.\n(نسخهٔ ریموت: {ver})",
                    parent=parent))
            return

        # نمایش دیالوگ تأیید در main thread
        names = "\n".join(f"  • {u['download_name']}  (v{u['remote_version']})" for u in updates)
        def ask_and_download():
            ok = messagebox.askyesno(
                "نسخهٔ جدید اکسل",
                f"{len(updates)} فایل اکسل روی گیت‌هاب جدیدتر است:\n\n{names}\n\n"
                "آیا می‌خواهید همین الان دانلود و جایگزین شوند؟",
                parent=parent)
            if not ok:
                return
    ...
    def _run_excel_download(self, updates, base, token, parent):
        """دانلود فایل‌ها با نوار پیشرفت ساده (پنجرهٔ modal)."""
        progress = tk.Toplevel(parent)
        progress.title("دانلود به‌روزرسانی")
        progress.transient(parent)
        progress.grab_set()
        progress.geometry("420x140")
        progress.resizable(False, False)
        lbl = tk.Label(progress, text="در حال دانلود…", font=(FONT_FA, 10), justify="right")
        ...
        def do_download():
            local_versions = dict(self.config.get(CFG_LOCAL_EXCEL_VERSIONS) or {})
            errors = []
            for i, u in enumerate(updates):
                ...
                try:
                    ...
                    local_versions[u["rel_path"]] = {
                        "version": u["remote_version"],
                        "sha256": sha or u.get("remote_sha256") or "",
                    }
                except Exception as e:
                    errors.append(f"{u['download_name']}: {e}")
            self.config[CFG_LOCAL_EXCEL_VERSIONS] = local_versions
            ...
            def finish():
                ...
                if errors:
                    messagebox.showwarning(
                        "به‌روزرسانی ناقص",
                        "برخی فایل‌ها دانلود نشدند:\n\n" + "\n".join(errors),
                        parent=parent)
                else:
                    messagebox.showinfo(
                        "موفق",
                        f"{len(updates)} فایل با موفقیت به‌روز شد.\n"
                        "دادهٔ برنامه دوباره بارگذاری می‌شود.",
                        parent=parent)
    ...
    def _load_motorcycle_data(self):
        """دیتای موتورسیکلت رو از فایل 18_MotorCycle.xlsm توی پوشه‌ی دیاگ
        می‌خونه. این تابع هر بار که برنامه باز می‌شه صدا زده می‌شه (نه فقط
        بار اول) تا همیشه آخرین نسخه‌ی فایل اکسل خونده بشه و کاربر مجبور
        نباشه دستی رفرش بزنه. اگه پوشه/فایل پیدا نشه، به tree_data.json
        پیش‌فرضِ همراه برنامه برمی‌گرده (برای تست/توسعه)."""
        path = os.path.join(self.diag_folder, MOTORCYCLE_EXCEL_NAME)
        if os.path.exists(path):
            try:
                self.motorcycle_data = TreeData.from_excel_file(path)
                self._motorcycle_source_desc = MOTORCYCLE_EXCEL_NAME
                return True
            except Exception as e:
                messagebox.showwarning(
                    "خطا در خواندن اکسل موتورسیکلت",
                    f"{path}\n\n{e}\n\nدر صورت وجود، از دیتای پیش‌فرض استفاده می‌شه."
                )

        data_path = resource_path(DATA_FILE)
        if os.path.exists(data_path):
            try:
                self.motorcycle_data = TreeData.from_json_file(data_path)
                self._motorcycle_source_desc = "Default (tree_data.json)"
                return True
            except Exception as e:
                messagebox.showerror("خطا", f"خطا در خواندن دیتای پیش‌فرض:\n{e}")
        self.motorcycle_data = None
        return False

    def _load_car_maker(self, label):
        """دیتای یک شرکت خودروساز خاص رو (فقط وقتی اولین بار انتخاب بشه)
        از فایل اکسل مخصوصش می‌خونه و توی self.car_maker_data کش می‌کنه."""
        ...
        if not os.path.exists(path):
            messagebox.showwarning("فایل پیدا نشد",
                                    f"فایل اکسل «{label}» پیدا نشد:\n{path}\n\n"
                                    "از کارت «تنظیمات» مسیر پوشه رو بررسی کنید.")
            return None
        try:
    ...
    def _switch_category(self, value):
        ...
        if value == self.CATEGORY_MOTORCYCLE:
            if self.motorcycle_data is None:
                messagebox.showwarning("داده‌ای نیست",
                                        "دیتای موتورسیکلت پیدا/خونده نشد.\n"
                                        "از کارت «تنظیمات» مسیر پوشه رو بررسی کنید.")
                return False
    ...
    def open_settings(self):
        ...
        card_w = min(460, max(340, int(W * 0.78)))
        card_h = min(520, max(400, int(H * 0.78)))
        card_x = (W - card_w) // 2
        ...
        tk.Label(body, text="پوشه مورد نظر را انتخاب کنید",
                 font=(FONT_FA, 9), bg="#ffffff", fg="#64748b",
                 anchor="e").pack(fill="x", pady=(0, 12))

        def make_path_card(parent, icon, title, subtitle, on_browse):
            """کارت انتخاب پوشه — بدون نمایش مسیر."""
            wrap = tk.Frame(parent, bg="#f8fafc",
                             highlightbackground="#e2e8f0", highlightthickness=1)
            wrap.pack(fill="x", pady=(0, 12))

            top = tk.Frame(wrap, bg="#f8fafc")
            top.pack(fill="x", padx=12, pady=12)

            icon_fr = tk.Frame(top, bg=DEV_HEADER_BG, width=36, height=36)
            icon_fr.pack(side="right")
            icon_fr.pack_propagate(False)
            tk.Label(icon_fr, text=icon, font=(FONT_EN_NAME, 14),
                     bg=DEV_HEADER_BG, fg="#ffffff").pack(expand=True)

            titles = tk.Frame(top, bg="#f8fafc")
            titles.pack(side="right", fill="x", expand=True, padx=(0, 10))
            tk.Label(titles, text=title, font=(FONT_FA, 11, "bold"),
                     bg="#f8fafc", fg="#0f172a", anchor="e").pack(fill="x")
            tk.Label(titles, text=subtitle, font=(FONT_EN_NAME, 8),
                     bg="#f8fafc", fg="#94a3b8", anchor="e").pack(fill="x")

            browse_btn = tk.Button(
                top, text="  انتخاب پوشه  ",
                font=(FONT_FA, 9, "bold"),
                ...
                command=on_browse, padx=12, pady=6)
            browse_btn.pack(side="left")

            def on_b_enter(_e):
                browse_btn.configure(bg=DEV_HEADER_HOVER)

            def on_b_leave(_e):
                browse_btn.configure(bg=DEV_HEADER_BG)

            browse_btn.bind("<Enter>", on_b_enter)
            browse_btn.bind("<Leave>", on_b_leave)
        ...
        def browse_diag():
            new_folder = filedialog.askdirectory(
                title="انتخاب پوشه‌ی فایل‌های اکسل دیاگ (Diag_Menu)",
                initialdir=self.diag_folder if os.path.isdir(self.diag_folder) else None,
                parent=self)
            if not new_folder:
                return
            self.diag_folder = new_folder
            self.config["diag_folder"] = new_folder
            save_config(self.config)
            self.car_maker_data = {}
            self.car_maker_source = {}
            self._load_motorcycle_data()
            messagebox.showinfo("موفق", "پوشه‌ی منوی دیاگ با موفقیت تنظیم شد.", parent=self)
            self.clear_all()

        def browse_database():
            new_folder = filedialog.askdirectory(
                title="انتخاب پوشه‌ی اکسل دیتابیس (Diag_Database)",
                initialdir=self.database_folder if os.path.isdir(self.database_folder) else None,
                parent=self)
            if not new_folder:
                return
            self.database_folder = new_folder
            self.config["database_folder"] = new_folder
            save_config(self.config)
            self._update_database_shortcut()
            messagebox.showinfo("موفق", "پوشه‌ی دیتابیس با موفقیت تنظیم شد.", parent=self)

        make_path_card(body, "📁", "منوی دیاگ", "Diag_Menu", browse_diag)
        make_path_card(body, "🗄", "دیتابیس", "Database", browse_database)

        # --- بخش به‌روزرسانی از گیت‌هاب ---
        gh_wrap = tk.Frame(body, bg="#f8fafc",
                           highlightbackground="#e2e8f0", highlightthickness=1)
        gh_wrap.pack(fill="x", pady=(4, 8))
        tk.Label(gh_wrap, text="به‌روزرسانی اکسل از گیت‌هاب",
                 font=(FONT_FA, 10, "bold"), bg="#f8fafc", fg="#0f172a",
                 anchor="e").pack(fill="x", padx=12, pady=(10, 2))
        tk.Label(gh_wrap,
                 text="آدرس raw ریپو (بدون / انتهایی)",
                 font=(FONT_FA, 8), bg="#f8fafc", fg="#94a3b8",
                 anchor="e").pack(fill="x", padx=12)
        gh_var = tk.StringVar(value=self.config.get(CFG_GITHUB_RAW_BASE) or "")
        gh_entry = tk.Entry(gh_wrap, textvariable=gh_var, font=(FONT_EN_NAME, 9),
                            justify="left", relief="solid", bd=1)
        gh_entry.pack(fill="x", padx=12, pady=(4, 6))
        tk.Label(gh_wrap,
                 text="توکن (فقط برای ریپوی خصوصی — اختیاری)",
                 font=(FONT_FA, 8), bg="#f8fafc", fg="#94a3b8",
                 anchor="e").pack(fill="x", padx=12)
        token_var = tk.StringVar(value=self.config.get(CFG_GITHUB_TOKEN) or "")
        token_entry = tk.Entry(gh_wrap, textvariable=token_var, font=(FONT_EN_NAME, 9),
                               justify="left", relief="solid", bd=1, show="•")
        token_entry.pack(fill="x", padx=12, pady=(4, 8))

        def save_github_settings():
            self.config[CFG_GITHUB_RAW_BASE] = gh_var.get().strip()
            self.config[CFG_GITHUB_TOKEN] = token_var.get().strip()
            save_config(self.config)

        def on_check_updates():
            save_github_settings()
            threading.Thread(
                target=lambda: self._check_excel_updates(silent_if_none=False, parent=self),
                daemon=True).start()

        btn_row = tk.Frame(gh_wrap, bg="#f8fafc")
        btn_row.pack(fill="x", padx=12, pady=(0, 12))
        save_gh_btn = tk.Button(
            btn_row, text="ذخیره آدرس", font=(FONT_FA, 9, "bold"),
            bg="#64748b", fg="#ffffff", activebackground="#475569",
            activeforeground="#ffffff", bd=0, relief="flat",
            cursor=self.click_cursor, command=save_github_settings, padx=10, pady=5)
        save_gh_btn.pack(side="left")
        check_btn = tk.Button(
            btn_row, text="بررسی به‌روزرسانی", font=(FONT_FA, 9, "bold"),
            bg=DEV_HEADER_BG, fg="#ffffff", activebackground=DEV_HEADER_HOVER,
            activeforeground="#ffffff", bd=0, relief="flat",
            cursor=self.click_cursor, command=on_check_updates, padx=10, pady=5)
